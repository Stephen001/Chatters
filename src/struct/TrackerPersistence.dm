...
TrackerDB/Database
	...
	findByIP(ip)
		var/TrackerEntry/entry = new()
		var/list/results = server_manager.database.select("SELECT DISTINCT ipaddress FROM USER_IP_ADDRESS WHERE ipaddress = [server_manager.database.quote(ip)]")
		if (length(results["ipaddress"]))
			entry.ips.Add(results["ipaddress"])
		return __expandEntry(entry)

	findByCkey(ckey)
		var/TrackerEntry/entry = new()
		var/list/results = server_manager.database.select("SELECT DISTINCT ckey FROM USER_IP_ADDRESS WHERE ckey = [server_manager.database.quote(ckey)]")
		if (length(results["ckey"]))
			entry.ckeys.Add(results["ckey"])
		return __expandEntry(entry)

	findByCID(cid)
		var/TrackerEntry/entry = new()
		var/list/results = server_manager.database.select("SELECT DISTINCT computerid FROM USER_COMPUTER_ID WHERE computerid = [server_manager.database.quote(cid)]")
		if (length(results["computerid"]))
	...
	proc
		__expandEntry(var/TrackerEntry/entry)
			if(length(entry.ckeys) || length(entry.ips) || length(entry.cids))
				...
				if(length(entry.ckeys))
					var/list/results = server_manager.database.select("SELECT DISTINCT * FROM USER_IP_ADDRESS WHERE ckey IN [__sqlList(entry.ckeys)]")
					if (length(results["ckey"]))
						__setUnion(ckeys, results["ckey"])
					if (length(results["ipaddress"]))
						__setUnion(ips, results["ipaddress"])
					results = server_manager.database.select("SELECT DISTINCT * FROM USER_COMPUTER_ID WHERE ckey IN [__sqlList(entry.ckeys)]")
					if (length(results["ckey"]))
						__setUnion(ckeys, results["ckey"])
					if (length(results["computerid"]))
						__setUnion(cids, results["computerid"])
				if(length(entry.ips))
					var/list/results = server_manager.database.select("SELECT DISTINCT * FROM USER_IP_ADDRESS WHERE ipaddress IN [__sqlList(entry.ips)]")
					if (length(results["ckey"]))
						__setUnion(ckeys, results["ckey"])
					if (length(results["ipaddress"]))
						__setUnion(ips, results["ipaddress"])
				if (length(entry.cids))
					var/list/results = server_manager.database.select("SELECT DISTINCT * FROM USER_COMPUTER_ID WHERE computerid IN [__sqlList(entry.cids)]")
					if (length(results["ckey"]))
						__setUnion(ckeys, results["ckey"])
					if (length(results["computerid"]))
						__setUnion(cids, results["computerid"])
				__setUnion(entry.ckeys, ckeys)
				__setUnion(entry.ips, ips)
		...
		__sqlList(var/list/L)
			...
			for (var/i = 1; i <= length(L); i++)
				result += "[server_manager.database.quote(L[i])]"
				if (i < length(L))
					result += ", "
			result += ")"
			return result
