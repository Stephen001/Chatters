...
TrackerDB/Database
	...
	findByIP(ip)
		var/TrackerEntry/entry = new()
		var/list/results = __sqlList(server_manager.database.select("SELECT DISTINCT ipaddress FROM USER_IP_ADDRESS WHERE ipaddress = [server_manager.database.quote(ip)]"))
		if (length(results))
	...
	findByCkey(ckey)
		var/TrackerEntry/entry = new()
		var/list/results = __sqlList(server_manager.database.select("SELECT DISTINCT ckey FROM USER_IP_ADDRESS WHERE ckey = [server_manager.database.quote(ckey)]"))
		if (length(results))
	...
	findByCID(cid)
		var/TrackerEntry/entry = new()
		var/list/results = __sqlList(server_manager.database.select("SELECT DISTINCT computerid FROM USER_COMPUTER_ID WHERE computerid = [server_manager.database.quote(cid)]"))
		if (length(results))
		...
		__expandEntry(var/TrackerEntry/entry)
			if(length(entry.ckeys) || length(entry.ips) || length(entry.cids))
				...
				if(length(entry.ckeys))
					var/list/results = __sqlList(server_manager.database.select("SELECT DISTINCT * FROM USER_IP_ADDRESS WHERE ckey IN [__sqlList(entry.ckeys)]"))
					ckeys.Add(results["ckey"])
					ips.Add(results["ipaddress"])
					results = __sqlList(server_manager.database.select("SELECT DISTINCT * FROM USER_COMPUTER_ID WHERE ckey IN [__sqlList(entry.ckeys)]"))
					ckeys.Add(results["ckey"])
					cids.Add(results["computerid"])
				if(length(entry.ips))
					var/list/results = __sqlList(server_manager.database.select("SELECT DISTINCT * FROM USER_IP_ADDRESS WHERE ipaddress IN [__sqlList(entry.ips)]"))
					ckeys.Add(results["ckey"])
					ips.Add(results["ipaddress"])
				if (length(entry.cids))
					var/list/results = __sqlList(server_manager.database.select("SELECT DISTINCT * FROM USER_COMPUTER_ID WHERE computerid IN [__sqlList(entry.cids)]"))
					ckeys.Add(results["ckey"])
					cids.Add(results["computerid"])
